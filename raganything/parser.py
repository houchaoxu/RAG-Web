...
class Parser:
    ...
    # ── Office → Markdown helpers (shared by all parsers) ──────────

    def _docx_to_markdown(self, doc_path: Path) -> str:
    ...
    def _pptx_to_markdown(self, doc_path: Path) -> str:
    ...
    def _xlsx_to_markdown(self, doc_path: Path) -> str:
    ...
    def _office_to_markdown(self, doc_path: Path) -> str:
        """Route office file to the appropriate converter."""
        ext = doc_path.suffix.lower()
        if ext == ".docx":
            return self._docx_to_markdown(doc_path)
        elif ext == ".pptx":
            return self._pptx_to_markdown(doc_path)
        elif ext == ".xlsx":
            return self._xlsx_to_markdown(doc_path)
        raise ValueError(f"No native converter for {ext}")
...
class MineruParser(Parser):
    ...
    def _run_mineru_command(
        ...
        **kwargs,
        ...
        """
        # Default to 'pipeline' backend for speed — VLM backends (hybrid-auto-engine,
        # vlm-auto-engine) are very slow on machines without a dedicated GPU.
        # Users can override by passing backend="hybrid-auto-engine" explicitly.
        if not backend:
            backend = "pipeline"
        ...
        try:
            ...
            # Limit MinerU's thread usage to prevent system lockup on multi-core machines.
            # Without this, OpenMP/PyTorch defaults to all cores, freezing the UI.
            max_threads = max(2, (os.cpu_count() or 4) // 2)
            for var in ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS"):
                env.setdefault(var, str(max_threads))
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        try:
            ...
            # Inject default backend from config if not explicitly provided
            if "backend" not in kwargs or not kwargs["backend"]:
                config_backend = getattr(self, "config", None)
                if config_backend and hasattr(config_backend, "mineru_backend"):
                    kwargs["backend"] = config_backend.mineru_backend
    ...
    def parse_office_doc(
        ...
        **kwargs,
        ...
        """
        Parse office document directly to Markdown.

        Modern formats (.docx, .pptx, .xlsx) are parsed natively.
        Legacy formats (.doc, .ppt, .xls) fall back to PDF conversion + MinerU.
        """
        doc_path = Path(doc_path)
        if not doc_path.exists():
            raise FileNotFoundError(f"Office file does not exist: {doc_path}")

        ext = doc_path.suffix.lower()
        try:
            if ext in (".docx", ".pptx", ".xlsx"):
                md = self._office_to_markdown(doc_path)
                self.logger.info(f"Parsed {ext} directly to Markdown: {doc_path} ({len(md)} chars)")
                return [{"type": "text", "text": md, "page_idx": 0}]
            else:
                self.logger.info(f"Legacy format {ext}, falling back to PDF conversion")
    ...
    def parse_text_file(
        ...
        **kwargs,
        ...
        """
        Parse text/markdown files by reading content directly.

        Supported formats: .txt, .md

        Args:
            text_path: Path to the text file (.txt, .md)
            output_dir: Output directory path (unused for text files)
            lang: Document language (unused for text files)
            **kwargs: Additional parameters (unused for text files)
        ...
        try:
            ...
            self.logger.info(f"Read text file directly: {text_path} ({len(content)} chars)")
    ...
    def _run_docling_command(
        ...
        **kwargs,
        ...
        try:
            ...
            # Limit thread usage to prevent system lockup
            max_threads = max(2, (os.cpu_count() or 4) // 2)
            for var in ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS"):
                env.setdefault(var, str(max_threads))
...
class PaddleOCRParser(Parser):
    ...
    def parse_text_file(
        ...
        **kwargs,
        ...
        content = text_path.read_text(encoding="utf-8")
        self.logger.info(f"Read text file directly: {text_path} ({len(content)} chars)")
        return [{"type": "text", "text": content, "page_idx": 0}]
